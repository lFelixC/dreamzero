"""Shared logic for the DreamZero AR websocket server entrypoints.

Two entrypoints consume this module:

* ``dreamzero/socket_test_optimized_AR.py`` -- the *legacy native* AR server.
  It speaks DreamZero/DROID's native wire protocol directly: clients send
  ``video.*`` / ``state.*`` / ``annotation.*`` dicts and the server returns an
  ``action.*`` dict. The first frame the server sends is a plain
  ``policy_metadata`` dict (NOT a RoboArena ``PolicyServerConfig``).

* ``dreamzero/socket_test_robolab_AR.py`` -- the *RoboLab / RoboArena* server.
  It wraps the policy in :class:`ARDroidRoboarenaPolicy`, speaks the RoboArena
  websocket protocol (``observation/*`` + ``session_id`` + ``endpoint``), and
  sends a full :class:`PolicyServerConfig` on connect.

Everything protocol-agnostic lives here so the two entrypoints stay thin and
the ~2000-line implementation is not duplicated. Run from the ``dreamzero``
repo root so the ``eval_utils`` namespace package resolves.
"""
...
# ---------------------------------------------------------------------------
# Architecture / checkpoint helpers
# ---------------------------------------------------------------------------
...
class _ArchitectureArgs:
    """Subset of entrypoint Args used by architecture resolution."""

    model_path: str
    architecture: Literal["auto", "joint", "mot"]
    allow_architecture_override: bool


def _resolve_model_path_for_architecture(
    args: _ArchitectureArgs,
...
# ---------------------------------------------------------------------------
# Action-head temporal state (per-session KV-cache / causal state)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RoboArena wrapper policy (observation/* -> DreamZero DROID -> actions)
# ---------------------------------------------------------------------------
...
class ARDroidRoboarenaPolicy:
    ...
    def _convert_observation(self, obs: dict) -> dict:
        ...
        # NOTE: This fallback exists for generic RoboArena compatibility. The
        # RoboLab default path always sends both exterior cameras (the runner
        # registers the right over-shoulder camera by default), so it never
        # relies on this duplication.
    ...
    def _snapshot_wrapper_state(
        ...
        video_across_time: list[torch.Tensor],
    ) -> ARSessionState:
    ...
    def _postprocess_group_actions(
        ...
        for batch_index, prepared in enumerate(prepared_items):
            ...
            if video_pred_to_save is not None:
                video_chunk = (
                    video_pred_to_save[batch_index : batch_index + 1]
                    if batch_size > 1
                    else video_pred_to_save
                )
...
# ---------------------------------------------------------------------------
# Native websocket server (DreamZero/DROID video.*/state.*/annotation.* wire
# protocol). Also owns the distributed worker loop reused by both entrypoints.
# ---------------------------------------------------------------------------

class WebsocketPolicyServer:
    """Serves a policy using the native DreamZero/DROID websocket protocol.

    Unlike the RoboArena ``eval_utils.policy_server.WebsocketPolicyServer``, this
    server:

    * sends a plain ``metadata`` dict (NOT a RoboArena ``PolicyServerConfig``)
      on connect,
    * receives native ``video.*`` / ``state.*`` / ``annotation.*`` observations
      directly (no ``endpoint`` field, no ``observation/`` prefix),
    * returns the ``action.*`` dict straight from the policy (no
      ``{"actions": ...}`` wrapping).

    It also implements ``_worker_loop`` / ``_receive_batch_from_rank0`` used by
    non-rank-0 processes in *both* entrypoints, because the worker loop only
    drives the distributed forward pass and is protocol-agnostic.
    """
...
# ---------------------------------------------------------------------------
# Distributed mesh + policy transform patching
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Shared policy loading for both entrypoints
# ---------------------------------------------------------------------------

@dataclasses.dataclass
class LoadedPolicy:
    """Bundle of everything both server entrypoints need after model load."""
    ...
    loaded_architecture: str | None
    mot_inference_video_mode: str | None
    cache_order_sensitive: bool
    supports_batching: bool
    output_dir: str | None
    # Holds the TemporaryDirectory created by an architecture override so it is
    # NOT garbage-collected (and its symlinks/config wiped) while the server is
    # still serving from the patched checkpoint view. The caller must keep the
    # whole LoadedPolicy alive for the lifetime of the server, exactly as the
    # pre-refactor main() kept it in a local.
    architecture_override_tmp: tempfile.TemporaryDirectory | None = None


@dataclasses.dataclass
class _LoadArgs:
    """Subset of entrypoint Args used by load_policy."""
...
def load_policy(args: _LoadArgs) -> LoadedPolicy:
    """Shared model-load + distributed-init + resolution-patching pipeline.

    Both the legacy native AR server and the RoboLab/RoboArena server call this
    so the ~80 lines of checkpoint resolution, mesh/signal-group setup, policy
    construction, and transform patching are not duplicated.
    """
    ...
    # Increase the recompile limit for inference due to autoregressive nature
    # of the model (several possible shapes).
    torch._dynamo.config.recompile_limit = 800

    embodiment_tag = "oxe_droid"
    model_path = args.model_path
    load_model_path, effective_architecture, architecture_override_tmp = _resolve_model_path_for_architecture(args)
    ...
    # Output directory for videos (rank 0 only).
    if rank == 0:
    ...
    return LoadedPolicy(
        ...
        architecture_override_tmp=architecture_override_tmp,
    )
