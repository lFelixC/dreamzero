...
class Args:
    # Hardware parameters
    # DROID keys use "left" for the camera stream; these ids describe physical exterior viewpoints.
    right_camera_id: str = "36517165"
    left_camera_id: str | None = None
    wrist_camera_id: str = "13337231"
    missing_left_camera_strategy: Literal["duplicate_right", "mask_left"] = "mask_left"
...
def _build_input_video_frame(
    ...
    left_exterior = image_tools.resize_with_pad(left_image, height, width)
    wrist = image_tools.resize_with_pad(wrist_image, height, width)
    right_exterior = image_tools.resize_with_pad(right_image, height, width)
    return np.concatenate([left_exterior, wrist, right_exterior], axis=1)


def _resolve_left_image(
    args: Args,
    right_image: np.ndarray,
    left_image: np.ndarray | None,
    warned_missing_left: bool,
) -> tuple[np.ndarray, bool]:
    if left_image is not None:
        return left_image, warned_missing_left

    if not warned_missing_left:
        if args.left_camera_id is None:
            missing_left_message = "Left camera id not provided"
        else:
            missing_left_message = f"Left camera '{args.left_camera_id}' not found in observations"

        if args.missing_left_camera_strategy == "mask_left":
            print(
                f"{missing_left_message}, filling DreamZero exterior_image_1_left with a black mask."
            )
        else:
            print(
                f"{missing_left_message}, duplicating right camera into DreamZero exterior_image_1_left."
            )
        warned_missing_left = True

    if args.missing_left_camera_strategy == "mask_left":
        return np.zeros_like(right_image), warned_missing_left
    return right_image, warned_missing_left
...
def _extract_observation(args: Args, obs_dict: dict) -> dict:
    ...
    for key, value in image_observations.items():
        if args.right_camera_id in key and "left" in key:
            right_image = value
        elif args.left_camera_id and args.left_camera_id in key and "left" in key:
            left_image = value
        elif args.wrist_camera_id in key and "left" in key:
            wrist_image = value

    if right_image is None:
        raise ValueError(f"Could not find right camera '{args.right_camera_id}' in observation keys: {list(image_observations)}")
...
class DreamZeroRealPolicyClient:
    def __init__(
        ...
        self._rtc_delay_step_scale = 1 + self._interpolation_substeps
        self._client_lock = threading.Lock()
        self._prefetch_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="dreamzero_prefetch")

        self._history = {
            "right": deque(maxlen=FRAME_HISTORY_LENGTH),
            "left": deque(maxlen=FRAME_HISTORY_LENGTH),
    ...
    def _append_history(self, right_image: np.ndarray, left_image: np.ndarray, wrist_image: np.ndarray) -> None:
        self._history["right"].append(right_image)
        self._history["left"].append(left_image)
    ...
    def _interpolate_action_pair(
        ...
        # Keep the previous gripper target during inserted interpolation steps for stability.
    ...
    def _compute_measured_delay_steps(self, policy_roundtrip_ms: float) -> int:
        ...
        delay_in_control_steps = policy_roundtrip_ms / control_period_ms
        delay_in_rtc_steps = delay_in_control_steps / float(max(self._rtc_delay_step_scale, 1))
        return max(int(np.ceil(delay_in_rtc_steps)), 0)
    ...
    def _activate_chunk(self, result: dict[str, object]) -> int:
        pred_action_chunk = np.asarray(result["pred_action_chunk"], dtype=np.float32)
        self._set_action_chunk(pred_action_chunk)
        if self._use_rtc and self._rtc_use_measured_delay:
    ...
    def _should_launch_prefetch(self) -> bool:
        ...
        if self._base_actions_from_chunk_completed < launch_threshold:
            return False
        return len(self._history["right"]) >= FRAME_HISTORY_LENGTH
    ...
    def infer(
        ...
        self._append_history(right_image, left_image, wrist_image)
        ...
        if self._should_launch_prefetch():
            ...
            if self._use_rtc:
                request_data["rtc_step_idx"] = np.int32(request_step_idx)
                request_data["rtc_inference_delay_steps"] = np.int32(request_rtc_delay_steps)
        ...
        if self._pred_action_chunk is None or self._actions_from_chunk_completed >= len(self._pred_action_chunk):
            ...
            if promoted_pending:
                ...
                if self._use_rtc:
                    request_data["rtc_step_idx"] = np.int32(self._episode_step_idx)
                    request_data["rtc_inference_delay_steps"] = np.int32(request_rtc_delay_steps)
                build_request_ms += (time.perf_counter() - build_request_start) * 1000
                wait_start = time.perf_counter()
                with prevent_keyboard_interrupt():
    ...
    while True:
        ...
        for t_step in bar:
            ...
            try:
                ...
                left_image = curr_obs["left_image"]
                left_image, warned_missing_left = _resolve_left_image(
                    args,
                    right_image,
                    left_image,
                    warned_missing_left,
