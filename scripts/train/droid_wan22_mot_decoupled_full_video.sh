...
OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/droid_unidir_static_one_step_224x448_run1}"
...
ACTION_EXPERT_WEIGHT_DECAY="${ACTION_EXPERT_WEIGHT_DECAY:-1e-2}"

IMAGE_RESOLUTION_WIDTH="${IMAGE_RESOLUTION_WIDTH:-448}"
IMAGE_RESOLUTION_HEIGHT="${IMAGE_RESOLUTION_HEIGHT:-224}"
MODEL_TARGET_HEIGHT="${MODEL_TARGET_HEIGHT:-224}"
MODEL_TARGET_WIDTH="${MODEL_TARGET_WIDTH:-448}"
MODEL_FRAME_SEQLEN="${MODEL_FRAME_SEQLEN:-98}"
...
DATASET_SHARD_SCHEDULE_STRATEGY="${DATASET_SHARD_SCHEDULE_STRATEGY:-permutation}"
...
DYNAMICS_LOSS_WEIGHT="${DYNAMICS_LOSS_WEIGHT:-200.0}"
...
MOT_INFERENCE_VIDEO_MODE="${MOT_INFERENCE_VIDEO_MODE:-decoupled_denoise}"
MOT_DECOUPLE_VIDEO_ACTION_NOISE="${MOT_DECOUPLE_VIDEO_ACTION_NOISE:-true}"
MOT_DECOUPLED_INFERENCE_VIDEO_FINAL_NOISE="${MOT_DECOUPLED_INFERENCE_VIDEO_FINAL_NOISE:-0.0}"
MOT_DECOUPLED_INFERENCE_VIDEO_REFRESH_STEPS="${MOT_DECOUPLED_INFERENCE_VIDEO_REFRESH_STEPS:-1}"
# Default training keeps full-video MoT for the video branch and observed/history
# context, but masks the current future video latent from the action branch.
ENFORCE_ACTION_FUTURE_VIDEO_MASK="${ENFORCE_ACTION_FUTURE_VIDEO_MASK:-true}"
STATIC_VIDEO_INIT="${STATIC_VIDEO_INIT:-true}"
STATIC_VIDEO_INIT_NOISE="${STATIC_VIDEO_INIT_NOISE:-0.0}"
DROID_RANDOM_DROP_EXTERIOR_VIEW_PROB="${DROID_RANDOM_DROP_EXTERIOR_VIEW_PROB:-0.1}"

if [[ "${ENFORCE_ACTION_FUTURE_VIDEO_MASK,,}" == "true" ]]; then
  if [[ "${STATIC_VIDEO_INIT,,}" != "true" ]]; then
    echo "ERROR: ENFORCE_ACTION_FUTURE_VIDEO_MASK=true requires STATIC_VIDEO_INIT=true." >&2
    exit 1
  fi
  if ! awk "BEGIN { exit !(${STATIC_VIDEO_INIT_NOISE} < 1.0) }"; then
    echo "ERROR: ENFORCE_ACTION_FUTURE_VIDEO_MASK=true requires STATIC_VIDEO_INIT_NOISE < 1.0." >&2
    exit 1
  fi
fi
...
echo "Using ACTION_EXPERT_WEIGHT_DECAY=${ACTION_EXPERT_WEIGHT_DECAY}"
...
echo "Using DATASET_SHARD_SAMPLING_RATE=${DATASET_SHARD_SAMPLING_RATE}"
echo "Using DATASET_SHARD_SAMPLING_STRATEGY=${DATASET_SHARD_SAMPLING_STRATEGY}"
echo "Using DATASET_SHARD_SAMPLING_BLOCK_SIZE=${DATASET_SHARD_SAMPLING_BLOCK_SIZE}"
echo "Using DATASET_SHARD_SCHEDULE_STRATEGY=${DATASET_SHARD_SCHEDULE_STRATEGY}"
echo "Using TRACK_LOSS_GRAD=${TRACK_LOSS_GRAD}, LOSS_GRAD_LOGGING_STEPS=${LOSS_GRAD_LOGGING_STEPS}"
echo "Using DYNAMICS_LOSS_WEIGHT=${DYNAMICS_LOSS_WEIGHT}, ACTION_LOSS_WEIGHT=${ACTION_LOSS_WEIGHT}"
...
echo "Using ENFORCE_ACTION_FUTURE_VIDEO_MASK=${ENFORCE_ACTION_FUTURE_VIDEO_MASK}"
echo "Using STATIC_VIDEO_INIT=${STATIC_VIDEO_INIT}"
echo "Using STATIC_VIDEO_INIT_NOISE=${STATIC_VIDEO_INIT_NOISE}"
...
"${LAUNCH_CMD[@]}" \
  ...
  static_video_init="${STATIC_VIDEO_INIT}" \
  static_video_init_noise="${STATIC_VIDEO_INIT_NOISE}" \
  ...
  trainer.action_expert_weight_decay="${ACTION_EXPERT_WEIGHT_DECAY}" \
  ...
  dataset_shard_schedule_strategy="${DATASET_SHARD_SCHEDULE_STRATEGY}" \
