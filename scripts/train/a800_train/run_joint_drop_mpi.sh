...
# AI Station/OpenMPI wrapper for the A800 joint-drop baseline.
# Submit this script as the MPI job script. It keeps the experiment settings in
# run_joint_drop_2node.sh and only maps MPI topology to torchrun topology.
...
export MASTER_PORT="${MASTER_PORT:-29440}"
export OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/dreamzero_droid_wan22_joint_drop_a800_mpi}"
...
exec bash "${SCRIPT_DIR}/run_joint_drop_2node.sh" "$@"
