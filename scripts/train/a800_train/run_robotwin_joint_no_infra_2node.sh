...
# A800 2-node RoboTwin Wan2.2 joint training baseline.
...
VIDEO_LAYOUT="${VIDEO_LAYOUT:-${ROBOTWIN_VIDEO_LAYOUT:-robotwin_tshape}}"
if [[ "${VIDEO_LAYOUT}" == "robotwin_tshape" ]]; then
  # T-shape layout matches Fast-WAM/LingBot-style RoboTwin preprocessing:
  #   [cam_high 320x256]
  #   [cam_left 160x128 | cam_right 160x128]
  # Wan VAE /16 plus DiT patch (2x2) gives 12*10 = 120 visual tokens/frame.
  IMAGE_RESOLUTION_WIDTH="${IMAGE_RESOLUTION_WIDTH:-320}"
  IMAGE_RESOLUTION_HEIGHT="${IMAGE_RESOLUTION_HEIGHT:-256}"
  TARGET_VIDEO_HEIGHT="${TARGET_VIDEO_HEIGHT:-${TARGET_HEIGHT:-${MODEL_TARGET_HEIGHT:-384}}}"
  TARGET_VIDEO_WIDTH="${TARGET_VIDEO_WIDTH:-${TARGET_WIDTH:-${MODEL_TARGET_WIDTH:-320}}}"
  FRAME_SEQLEN="${FRAME_SEQLEN:-${MODEL_FRAME_SEQLEN:-${FRAMESEQ:-120}}}"
else
  IMAGE_RESOLUTION_WIDTH="${IMAGE_RESOLUTION_WIDTH:-320}"
...
if [[ "${VIDEO_LAYOUT}" != "robotwin_tshape" && "${VIDEO_LAYOUT}" != "default" ]]; then
  echo "ERROR: VIDEO_LAYOUT must be 'robotwin_tshape' or 'default', got '${VIDEO_LAYOUT}'"
  exit 1
fi
...
MOT_ACTION_INIT_FROM_VIDEO="${MOT_ACTION_INIT_FROM_VIDEO:-false}"
...
echo "========== RoboTwin joint A800 2-node baseline launch =========="
...
echo "VIDEO_LAYOUT=${VIDEO_LAYOUT}"
...
echo "MOT_ACTION_INIT_FROM_VIDEO=${MOT_ACTION_INIT_FROM_VIDEO}"
echo "================================================================"
...
if [[ "${ARCH}" == "mot" ]]; then
  TRAIN_OVERRIDES+=(
    ...
    "mot_action_init_from_video=${MOT_ACTION_INIT_FROM_VIDEO}"
