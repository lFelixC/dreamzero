...
# AI Station/OpenMPI wrapper for the A800 full-video MoT experiment.
# Submit this script as the MPI job script. It keeps the experiment settings in
# run_mot_full_video_2node.sh and only maps MPI topology to torchrun topology.
...
export OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/dreamzero_droid_wan22_mot_a800_full_video_mpi}"
...
exec bash "${SCRIPT_DIR}/run_mot_full_video_2node.sh" "$@"
