...
# A800 2-node RoboTwin Wan2.2 joint training.
...
# Infra knobs default to off; pass DREAMZERO_SHARD_SCHEDULE_BALANCE=cost_grouped
# and DREAMZERO_SHARD_COST_PROFILE=... to enable cost-group scheduling.
...
OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/dreamzero_robotwin_wan22_joint_a800_2node}"
...
# Cost-grouped shard scheduling is opt-in. The profile must be generated for
# RoboTwin, not reused from DROID or another dataset.
...
case "${DREAMZERO_SHARD_SCHEDULE_BALANCE,,}" in
  "" | "0" | "false" | "no" | "off" | "none")
    ;;
  *)
...
echo "========== RoboTwin joint A800 2-node launch =========="
...
echo "DATALOADER_WARMUP_BATCHES=${DREAMZERO_DATALOADER_WARMUP_BATCHES}"
