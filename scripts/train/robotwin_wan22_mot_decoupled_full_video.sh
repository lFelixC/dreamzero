...
if [[ -z "${ROBOTWIN_DATA_ROOT:-}" ]]; then
  if [[ -f "${DATASET_ROOT}/meta/modality.json" ]]; then
    ROBOTWIN_DATA_ROOT="${DATASET_ROOT}"
  else
    ROBOTWIN_DATA_ROOT="${DATASET_ROOT}/robotwin_unified_dreamzero_180x320"
  fi
fi
OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_ROOT}/dreamzero_robotwin_wan22_mot_decoupled_full_video}"
...
if [[ ! -f "${ROBOTWIN_DATA_ROOT}/meta/modality.json" ]]; then
  ...
  echo "Set ROBOTWIN_DATA_ROOT to the converted RoboTwin LeRobot/DreamZero dataset root."
  exit 1
fi
if [[ "${EPISODE_FILTER_PATH}" != "null" && ! -f "${EPISODE_FILTER_PATH}" ]]; then
  echo "ERROR: EPISODE_FILTER_PATH not found: ${EPISODE_FILTER_PATH}"
  exit 1
fi
...
echo "Using ROBOTWIN_DATA_ROOT=${ROBOTWIN_DATA_ROOT}"
echo "Using EPISODE_FILTER_PATH=${EPISODE_FILTER_PATH}"
