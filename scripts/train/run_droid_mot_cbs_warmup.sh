...
# Warm up the DROID MoT run to the reference LR, then use the final checkpoint
# as the common starting point for CBS branch training.
...
CBS_RUN_ID="${CBS_RUN_ID:-$(date -u +%Y%m%d_%H%M%S)}"

# Reference experiment: global batch 128, target LR 1.5e-5, 100k-step schedule.
...
OUTPUT_DIR="${OUTPUT_DIR:-${CBS_ROOT}/${CBS_RUN_ID}/warmup_b${BASE_GLOBAL_BATCH_SIZE}_lr${BASE_LEARNING_RATE}_steps${WARMUP_STEPS}}"

WANDB_PROJECT_NAME="${WANDB_PROJECT_NAME:-dreamzero}"
PER_DEVICE_BS="${PER_DEVICE_BS:-16}"
MAX_STEPS="${MAX_STEPS:-${WARMUP_STEPS}}"
SAVE_STEPS="${SAVE_STEPS:-${WARMUP_STEPS}}"
...
export WANDB_NAME="${WANDB_NAME:-cbs_mot_droid_${CBS_RUN_ID}_warmup_b${BASE_GLOBAL_BATCH_SIZE}}"
export WANDB_TAGS="${WANDB_TAGS:-cbs,mot,droid,warmup,b-${BASE_GLOBAL_BATCH_SIZE},lr-${BASE_LEARNING_RATE},steps-${WARMUP_STEPS},action-loss-weight-${ACTION_LOSS_WEIGHT}}"
export WANDB_NOTES="${WANDB_NOTES:-CBS warmup: DROID MoT, global batch ${BASE_GLOBAL_BATCH_SIZE}, target LR ${BASE_LEARNING_RATE}, action loss weight ${ACTION_LOSS_WEIGHT}, warmup-only ${WARMUP_STEPS} steps. Branch runs load this checkpoint as pretrained weights.}"
...
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-${BASE_GLOBAL_BATCH_SIZE}}"
...
echo "Expected warmup checkpoint: ${OUTPUT_DIR}/checkpoint-${MAX_STEPS}"
echo "=========================================="
...
TRAIN_OVERRIDES=(
  ...
  "training_args.learning_rate=${BASE_LEARNING_RATE}"
  "lr_scheduler_type=linear"
  "training_args.lr_scheduler_type=linear"
  "warmup_ratio=1.0"
  "training_args.warmup_ratio=1.0"
  ...
  "+cbs.reference_max_steps=${REFERENCE_MAX_STEPS}"
  "+cbs.reference_warmup_ratio=${REFERENCE_WARMUP_RATIO}"
  "+cbs.warmup_steps=${WARMUP_STEPS}"
